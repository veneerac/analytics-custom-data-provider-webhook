...
public class CustomDataProvider implements AnalyticsCustomDataProvider {
    ...
    private static final String TOPIC = "SUBSCRIBER_TOPIC";
    private static final String SUBSCRIBER_CALLBACK_URL = "SUBSCRIBER_CALLBACK";
    ...
    @Override public Map<String, Object> getCustomProperties(Object context) {
        if (context instanceof MessageContext) {
            ...
            customProperties.put("foo", null);
            customProperties.put("topic", getTopic((MessageContext) context));
            customProperties.put("destination", getSubscriberCallbackUrl((MessageContext) context));
    ...
    private String getSubscriberCallbackUrl(MessageContext context) {
        if (context.getPropertyKeySet().contains(SUBSCRIBER_CALLBACK_URL)) {
            return (String) context.getProperty(SUBSCRIBER_CALLBACK_URL);
        }
        return null;
    }

    private String getTopic(MessageContext context) {
        if (context.getPropertyKeySet().contains(TOPIC)) {
            return (String) context.getProperty(TOPIC);
        }
        return null;
    }
